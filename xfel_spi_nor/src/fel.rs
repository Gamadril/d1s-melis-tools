...
impl Fel {
  ...
  fn fel_request(&mut self, ty: u32, addr: u32, len: u32) -> Result<()> {
    ...
    let bytes = unsafe {
      std::slice::from_raw_parts(&req as *const FelRequest as *const u8, std::mem::size_of::<FelRequest>())
    };
  ...
  fn send_usb_request(&mut self, ty: u16, length: u32) -> Result<()> {
    ...
    let bytes = unsafe {
      std::slice::from_raw_parts(&req as *const UsbRequest as *const u8, std::mem::size_of::<UsbRequest>())
    };
  ...
  fn bulk_send(&mut self, data: &[u8]) -> Result<()> {
    ...
    while off < data.len() {
      ...
      let wrote = self.hdl.write_bulk(self.ep_out, &data[off..off + n], TIMEOUT)?;
      off += wrote;
    }
    Ok(())
  }
